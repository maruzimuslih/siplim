<?php
	...
	class myPDF extends FPDF{
		function header(){
	        ...
	        $this->Cell(276,12,'LAPORAN LIMBAH MEDIS PER RUANGAN',0,0,'C');
	        $this->Ln();
	    ...
	    function viewTable($db){
	        ...
	        $no=0;
	        $stmt = $db->query('SELECT nama_ruangan, nama_petugas, SUM(jumlah_masuk) AS jumlah FROM limbah_masuk join ruangan on limbah_masuk.id_ruangan = ruangan.id_ruangan join petugas on limbah_masuk.id_petugas = petugas.id_petugas GROUP BY nama_ruangan, nama_petugas');
	        while($data = $stmt->fetch(PDO::FETCH_OBJ)){
	        	$no++;
	        	$this->SetX(29);
	            $this->Cell(30,10,$no,1,0,'C');
	            $this->Cell(70,10,$data->nama_ruangan,1,0,'L');
	            if($data->jumlah<1000){
